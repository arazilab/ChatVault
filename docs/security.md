...
The ChatGPT content script accepts one fixed operation, requests only the same-origin conversation listing endpoint, and validates the response as an array before returning it to the popup. When ChatGPT requires it, the script reads the short-lived access token from the same-origin session endpoint and uses it only in memory for that request. It does not persist, log, export, or send the token elsewhere.
