...
The ChatGPT content script accepts one fixed operation, requests only the same-origin conversation listing endpoint, and validates the response as an array before returning it to the popup. It does not expose cookies or request headers to extension code.
